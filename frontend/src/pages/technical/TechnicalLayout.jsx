...
function findLabel(key) {
  ...
  if (key === "workbench") return "Application Workbench";
  return "Dashboard";
}
...
function Sidebar({ activeKey, onNavigate, onClose }) {
  ...
  const [openSections, setOpenSections] = useState(() => parent ? { [parent]: true } : {});
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-100">
      <div className="px-5 pt-6 pb-5 flex-shrink-0">
        <div className="flex items-center gap-2">
          <FlaskConical size={18} className="text-[#39B5E0]" />
          <div className="text-gray-900 font-bold text-lg tracking-wide">E-PAAS</div>
        </div>
        <div className="text-gray-400 text-[9px] uppercase tracking-wider">Technical Officer Portal</div>
      </div>
        ...
        {NAV.map((item) => {
          ...
          if (item.children) {
            return (
              <div key={item.key}>
                <button
                  onClick={() => setOpenSections((s) => ({ ...s, [item.key]: !s[item.key] }))}
                  className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors text-left ${
                    isParentActive ? "bg-[#39B5E0]/10 text-[#39B5E0]" : "text-gray-500 hover:bg-gray-100 hover:text-gray-900"
                  }`}
                >
                ...
                {isOpen && (
                  <div className="ml-4 mt-0.5 mb-1 space-y-0.5 border-l border-gray-100 pl-2">
                    {item.children.map((child) => (
                      <button
                        key={child.key}
                        onClick={() => nav(child.key)}
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-left transition-colors ${
                          activeKey === child.key ? "bg-[#39B5E0]/10 text-[#39B5E0]" : "text-gray-500 hover:bg-gray-100 hover:text-gray-900"
                        }`}
                      >
                        <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${activeKey === child.key ? "bg-[#39B5E0]" : "bg-gray-300"}`} />
                        {child.label}
                      </button>
                    ))}
                  </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors text-left ${
                isActive ? "bg-[#39B5E0]/10 text-[#39B5E0]" : "text-gray-500 hover:bg-gray-100 hover:text-gray-900"
              }`}
            >
              <Icon size={15} className="flex-shrink-0" />
              {item.label}
...
export default function TechnicalLayout({ activeKey, onNavigate, children }) {
  ...
  useEffect(() => {
    ...
    api.get("/technical/notifications", { params: { limit: 1 } })
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white rounded-2xl my-3 mr-3 shadow-sm overflow-hidden">
        <header className="sticky top-0 z-20 bg-white border-b border-gray-100 px-4 sm:px-6 py-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[9px] uppercase tracking-widest text-gray-400 font-semibold">Technical Officer</div>
              <div className="text-sm font-bold text-black leading-tight">{findLabel(activeKey)}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 pl-1">
              <div className="w-8 h-8 rounded-full bg-black text-white text-xs font-bold flex items-center justify-center flex-shrink-0">{initials}</div>
            ...
            <button onClick={logout} className="hidden sm:flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-700 transition-colors ml-1">
              <LogOut size={13} />
            </button>
          </div>
        </header>
